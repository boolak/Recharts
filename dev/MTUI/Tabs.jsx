...
const Tabs = React.createClass({
  ...
  this.props.children.map(function(index, elem) {
    arr.push(<li onClick={this.handleClick} data-name={index.props.titleName} className={elem==this.state.defaultVal?'mt-tabs-active':''} key={elem}><a href="javascript: void(0)">{index.props.titleName}</a></li>);
  }.bind(this))
  return arr;
  },
  tabBody : function(){
    var arr = [];
  this.props.children.map(function(index, elem) {
    arr.push(<div className={elem==this.state.defaultVal?'mt-tabs-item mt-tabs-active':'mt-tabs-item'} key={elem}>{index}</div>);
  }.bind(this))
  return arr;
  },
  render: function() {
